...
output_png <- file.path(project_root, "docs", "figures", "epl-2025-26-points-by-matchday.png")
...
end_labels <- points_progression %>%
  ...
  mutate(
    ...
    x_label = max(points_progression$match_number, na.rm = TRUE) + 1.1
  )
...
subtitle <- sprintf(
  "Cumulative points after each club's match; %s matches through %s.",
  completed_count,
  format(latest_match_date, "%d %b %Y")
)

p <- ggplot(plot_df, aes(x = match_number, y = cumulative_points, colour = team)) +
  geom_hline(
  ...
  geom_point(
    ...
    aes(x = match_number, y = cumulative_points, colour = team),
    size = 0.9,
    alpha = 0.75
  ) +
  geom_segment(
  ...
  scale_x_continuous(
    breaks = seq(0, max(plot_df$match_number, na.rm = TRUE), by = 5),
    limits = c(0, max(plot_df$match_number, na.rm = TRUE) + 6),
